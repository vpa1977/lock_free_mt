...
void flipImageTGA(ImageTGA* img)
{
	...
	for (int line = 0; line < img->sizeY / 2; ++line)
	{
		...
		{
			unsigned char tmp = img->data[dstOffest + colBit];
...
void convertRGBToRGBA(ImageTGA* img)
{
...
void CTgaReadSection::DecodeTGA(void* inBuf, int size, CPreloaderSection_DecodeResponse& resp)
{								  
								
	const unsigned char* in = (const unsigned char*)inBuf;
	ImageTGA* pImage = loadTGA(in, size);
	if (!pImage) 
	{
		assert(0);
		return;
	...
	resp.m_Width = pImage->sizeX;
	resp.m_Height = pImage->sizeY;
	
	resp.m_ARGBData.resize( resp.m_Width * resp.m_Height * sizeof(unsigned long) );
	
	unsigned long* pSrc = (unsigned long*)pImage->data;
	unsigned long* pDst = (unsigned long*)&resp.m_ARGBData[0];
	unsigned long count =0;
	for (int i = 0 ; i <resp.m_Width * resp.m_Height ; i ++ ) 
	{
		resp.m_ARGBData[count] = pImage->data[i*pImage->channels  +2];count++;
		resp.m_ARGBData[count] = pImage->data[i*pImage->channels + 1];count++;
		resp.m_ARGBData[count] = pImage->data[i*pImage->channels ];count++;
		resp.m_ARGBData[count] =  pImage->channels == 4 ? pImage->data[i*pImage->channels+3] : 0xFF   ;count++;
