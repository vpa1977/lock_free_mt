...
#include "../include/section_preloader_interface.h"
...
extern "C"
{
	...
	//#include "../../libjpg/include/jinclude.h"		// include JPEG library declarations
	#include "../../libjpg/include/jpeglib.h"		// include JPEG library declarations
	#include "../../libjpg/include/jerror.h"		// JPEG error handler
}
...
void CJpgReadSection::DecodeJPG( void * inBuf , int size, CPreloaderSection_DecodeResponse& response ) 
{
			const char * inBuffer =  (const char*)(inBuf);
			...
			jpeg_memory_src(&cinfo,(const JOCTET *) inBuffer, size);
			...
			response.m_Height = cinfo.output_height;
			response.m_Width  = cinfo.output_width;
			
			response.m_ARGBData.resize(response.m_Height * response.m_Width * 4);
			long count = 0;

			for (int j = 0 ; j <=response.m_Height-1  ; j ++ ) 
			{
				for (int i = 0; i < response.m_Width*3; i += 3)
				{
							response.m_ARGBData[count] = (buffer[j][i+2]); count ++;
							response.m_ARGBData[count] = (buffer[j][i+1]);count ++;
							response.m_ARGBData[count] = (buffer[j][i]);count ++;
							response.m_ARGBData[count] = (1);			count ++;
