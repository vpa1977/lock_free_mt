...
bool CVertexLoader::Load(const CVFSSection_ReadResponse& response )
{
		...
		vecBuffer.resize(response.m_Size + 1);
		char * buffer = (char*) &(vecBuffer[0]);
...
//		char Buffer[1024];
		...
		out_rVBFormat.m_Tex0Dimension = CVBFormat::TEXCOORDSET_NONE;
		out_rVBFormat.m_Tex1Dimension = CVBFormat::TEXCOORDSET_NONE;
		out_rVBFormat.m_Tex2Dimension = CVBFormat::TEXCOORDSET_NONE;
		out_rVBFormat.m_Tex3Dimension = CVBFormat::TEXCOORDSET_NONE;
...
CVertexLoader::DecodeVBFormatString( char* in_pszBuffer, CVBFormat& out_rVBFormat )
{
	...
	while( token )
	{
		...
		{
			out_rVBFormat.m_Tex0Dimension = (CVBFormat::TEXCOORDSET_THICKNESS)2;
		}
		token = strtok(NULL," \t\n");
	}
}
