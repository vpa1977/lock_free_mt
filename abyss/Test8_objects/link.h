...
class CLinkTarget : public boost::noncopyable
{
...
protected:
	...
	// dirty hack
	virtual void NotificatorDied( CDeathNotificatorBase* ) = 0;
	// another dirty hack
	virtual void KickMe() = 0;
};

class CLinkOrigin : public boost::noncopyable
{
public:
	virtual void ConnectLink( CLinkTarget* in_pTarget ) = 0;
	virtual void DisconnectLink( CLinkTarget* in_pTarget ) = 0;
protected:
...
public:
	...
	void DisconnectLink( CLinkTarget* in_pTarget ) {
		assert(m_NFilled>0);
		size_t i = 0;
		for( ; i<m_NFilled; ++i) {
