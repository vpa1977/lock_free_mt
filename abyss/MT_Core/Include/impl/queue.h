...
template< class T >
class MSQueue : public chain_allocator
{
private:
...
public:	
	atomic_long m_size;
	// default constructor
	MSQueue() : chain_allocator(256)
	{
		m_size.set(0);
		node_t* pNode = new  ((node_t*)allocate(sizeof(node_t))) node_t();
		pNode->m_pchunk = get_chunk();
		Head.ptr = Tail.ptr = pNode;
	}
	~MSQueue()
	{
		// remove the dummy head
		Head.ptr->m_pchunk->Decrease();
		//delete Head.ptr;
	}
	...
	// Enqueue method
	void enqueue(const T& t, chain_allocator& alloc)
	{
		m_size.inc();
		
		// Allocate a new node from the free list
		node_t* pNode = new ((node_t*)alloc.allocate(sizeof(node_t))) node_t(); 
		pNode->m_pchunk = alloc.get_chunk();
