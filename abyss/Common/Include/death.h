...
protected:
	...
	void CallDeathNotification( CLinkTarget* in_pTarget, CLinkOrigin* in_pOrigin ) {
		in_pTarget->NotificatorDied( in_pOrigin );
	}
};
...
public:
	...
	virtual ~CDeathNotificator() {
		for( size_t i=0; i<GetLinksSize(); ++i ) {
			CallDeathNotification( GetNthLink(i), (CLinkOrigin*)this );
		}
		printf("CDeathNotificator(%p)::~CDeathNotificator()\n",this);
	}
protected:
